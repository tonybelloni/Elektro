...
{
    public partial class frmVideosFilmadosReport : Form
    {
        ...
        public frmVideosFilmadosReport()
        {
            InitializeComponent();
        }

        private void frmVideosFilmadosReport_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (tipo == 1)
                {
                    ...
                    var lista = ocorrencias.Select(l => new
                    {
                        ...
                        HORAS = Math.Round((double)(l.LISTA_VIDEOS.TEMPO_TOTAL_VIDEOS / 60 / 60), 2)
                    }).OrderBy(l => l.FILTRO).AsQueryable().ToList();

                    ReportDataSource ds = new ReportDataSource("DSVideosFilmadosReport", lista);
                ...
                else if (tipo == 2)
                {
                    ...
                    var lista = (from o in ocorrencias
                                 ...
                                 select new { FILTRO = regiao1.FirstOrDefault().EQUIPES.REGIAO1.DESCRICAO, HORAS = Math.Round((double)(regiao1.Sum(l => l.LISTA_VIDEOS.TEMPO_TOTAL_VIDEOS) / 60 / 60), 2) }).AsQueryable().ToList();
                ...
                else
                {
                    ...
                    var lista = (from o in ocorrencias
                                 ...
                                 select new { FILTRO = localidade1.FirstOrDefault().EQUIPES.LOCALIDADE1.DESCRICAO, HORAS = Math.Round((double)(localidade1.Sum(l => l.LISTA_VIDEOS.TEMPO_TOTAL_VIDEOS) / 60 / 60), 2) }).AsQueryable().ToList();
